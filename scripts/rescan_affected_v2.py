...
with ProcessPoolExecutor(max_workers=4) as pool:
    for ok, inst_name, obsdate, nfiles, elapsed, err in pool.map(process_one, sorted_pairs, chunksize=1):
        scanned += ok
        failed += not ok
        if ok:
            print(f"  OK  {inst_name} {obsdate}  ({nfiles} files, {elapsed:.1f}s)  [{scanned + failed}/{total}]")
        else:
            print(f"  FAIL {inst_name} {obsdate}  ({elapsed:.1f}s): {err}  [{scanned + failed}/{total}]")

total_elapsed = time.time() - total_start
avg = total_elapsed / total if total else 0
print(f"\nDone: {scanned} OK, {failed} failed in {total_elapsed:.1f}s ({total_elapsed/60:.1f} min, {avg:.1f}s avg per date)")
