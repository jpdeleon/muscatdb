...
def rescan_batch(batch):
    local_results = []
    for inst_name, obsdate in batch:
        start = time.time()
        try:
            ...
            if result:
                ...
                local_results.append((True, inst_name, obsdate, nfiles, elapsed, None))
            else:
                local_results.append((True, inst_name, obsdate, 0, elapsed, None))
        except Exception as e:
            elapsed = time.time() - start
            local_results.append((False, inst_name, obsdate, 0, elapsed, str(e)))
...
n_workers = 4
batches = list(chunk(sorted_pairs, max(1, total // n_workers)))

with ProcessPoolExecutor(max_workers=n_workers) as pool:
    for batch_results in pool.map(rescan_batch, batches):
        for ok, inst_name, obsdate, nfiles, elapsed, err in batch_results:
            done += 1
            if ok:
                scanned += 1
