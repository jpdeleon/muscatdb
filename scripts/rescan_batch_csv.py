...
print("[3/5] Gathering FITS file paths (from CSVs, no directory listing)...")
tasks = []
missing_fits = 0
for inst_name, obsdate in sorted_pairs:
    inst = INSTRUMENTS[inst_name]
    for ccd in range(inst.nccd):
        csv_path = f"/ut3/muscat/obslog/{inst_name}/{obsdate}/obslog-{inst_name}-{obsdate}-ccd{ccd}.csv"
        ...
        try:
            with open(csv_path) as f:
                ...
                for row in reader:
                    fname = row.get("FRAME", "").strip()
                    if not fname:
                        continue
                    fp = f"{inst.data_dir}/{obsdate}/{fname}.fits"
                    tasks.append((fp, inst, ccd, obsdate, inst_name))
        except Exception as e:
            print(f"  [warn] cannot read {csv_path}: {e}")

print(f"  Total FITS files to process: {len(tasks)} ({missing_fits} missing FITS files skipped)")
...
max_workers = 100
print(f"[4/5] Processing FITS headers in parallel ({max_workers} threads)...")
# NFS-I/O-bound (~30ms/file): threads block on read() with the GIL released,
# so concurrency can far exceed core count to hide network latency. Threads
# avoid per-task pickling and yield results in order for live progress.
# chunksize is ignored by ThreadPoolExecutor.map (kept for signature parity).
chunksize = 1
