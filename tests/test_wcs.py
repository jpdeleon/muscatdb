...
def test_solve_wcs_muscat2_vs_muscat3():
    ...
    m2_file = Path(os.environ.get(
        "MUSCAT_WCS_TEST_MUSCAT2",
        str(Path(INSTRUMENTS["muscat2"].data_dir) / "241001" / "MCT21_2410010091.fits"),
    ))
    if not m2_file.is_file():
        pytest.skip(f"MuSCAT2 test file {m2_file} not found")
    ...
    m3_file = Path(os.environ.get(
        "MUSCAT_WCS_TEST_MUSCAT3",
        str(
            Path(INSTRUMENTS["muscat3"].data_dir)
            / "250706"
            / "ogg2m001-ep02-20250706-0766-e91.fits"
        ),
    ))
    if not m3_file.is_file():
        pytest.skip(f"MuSCAT3 test file {m3_file} not found")
...
def test_wcs_with_gaia_query_scaling():
    ...
    image_path = Path(os.environ.get(
        "MUSCAT_WCS_TEST_CALIBRATED",
        str(Path.home() / "temp" / "muscat_test_calib_toi1266" / "MSCT0_2201310272_calibrated.fits"),
    ))
